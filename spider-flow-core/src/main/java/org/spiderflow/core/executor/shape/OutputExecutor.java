...
import org.spiderflow.core.utils.MyCSVPrinter;
...
public class OutputExecutor implements ShapeExecutor, SpiderListener {
     ...
     * 输出CSVPrinter节点变量
     */
    private Map<String, MyCSVPrinter> cachePrinter = new HashMap<>();
...
//		String key = context.getId() + "-" + node.getNodeId();
        String key = csvName + "-" + node.getNodeId();
        MyCSVPrinter printer = cachePrinter.get(key);
            ...
            if (printer == null) {
                synchronized (cachePrinter) {
                    ...
                    if (printer == null) {
			            ...
			            BufferedOutputStream os = new BufferedOutputStream(new FileOutputStream(csvName), 1024 * 100) ;
                        ...
                        CSVPrinter TempPrinter = new CSVPrinter(osw, format);
                        printer = new MyCSVPrinter(TempPrinter, key, context.taskId);
...
//            printer.printer.printRecord(records);

            synchronized (cachePrinter) {
                printer.FinalFlag = false;
                printer.printer.printRecord(records);
                printer.FinalFlag = true;
            }
        } catch (IOException e) {
    ...
    private void releasePrinters(SpiderContext context) {
        for (Iterator<Map.Entry<String, MyCSVPrinter>> iterator = this.cachePrinter.entrySet().iterator(); iterator.hasNext(); ) {
            Map.Entry<String, MyCSVPrinter> entry = iterator.next();
            MyCSVPrinter printer = entry.getValue();
            if (printer != null && printer.FinalFlag && Objects.equals(printer.taskId, context.taskId)) {
                try {
                    System.out.println("输出文件");
                    System.out.println(printer.key);
                    System.out.println(printer.taskId);
